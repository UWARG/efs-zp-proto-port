...
public:
	void enter(AttitudeManager *attMgr) {
		(void) attMgr;
	}
	void execute(AttitudeManager *attMgr);
	void exit(AttitudeManager *attMgr) {
		(void) attMgr;
	}
	static AttitudeState& getInstance();
	static bool isAutonomous(void) {
		return FetchInstructionsMode::_is_autonomous;
	}
	static Teleop_Instructions_t* getTeleopInstructions(void) {
		return &_teleop_instructions;
	}
private:
	...
	static bool receiveTeleopInstructions(AttitudeManager *attMgr);
	static bool isArmed();
...
public:
	...
	static AttitudeState& getInstance();
	static SFOutput_t* getSFOutput(void) {
		return &_sf_output;
	}
private:
...
public:
	...
	static AttitudeState& getInstance();
	static PID_Output_t* getPIDOutput(void) {
		return _pid_output;
	}
private:
	ControlLoopMode() {
